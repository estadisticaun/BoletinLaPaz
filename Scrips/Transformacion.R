...
# Poblaciones - Sede La Paz ---
AspirantesPRE <- Aspirantes %>% filter(INS_SEDE_NOMBRE == "De La Paz", TIPO_NIVEL == "Pregrado")
AdmitidosPRE <- Aspirantes %>% filter(INS_SEDE_NOMBRE == "De La Paz", TIPO_NIVEL == "Pregrado", ADMITIDO == "Sí")
...
# GraduadosPOS <- Graduados %>% filter(TIPO_NIVEL == "Postgrado", SEDE_NOMBRE_ADM == 'La Paz')

Docentes <- Docentes %>% filter(SEDE == 'La Paz')
Administrativos <- Administrativos %>% filter(SEDE == 'La Paz')
...
Adm_Pre_Carreras <- AdmitidosPRE %>% group_by(YEAR, SEMESTRE, PROGRAMA) %>% summarise(Total=n()) %>% select(YEAR, SEMESTRE, PROGRAMA, Total) 
